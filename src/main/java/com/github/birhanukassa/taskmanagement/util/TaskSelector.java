...
public class TaskSelector {
    ...
    public static NamedTypedValue<Task> promptUserForTaskSelection(List<Task> tasks) {
        ...
        do {
            try {
                userTaskSelection = InputHandler.getUserInput("Enter task key or 'E' to exit: ", String.class);

                if (userTaskSelection.getValue().equalsIgnoreCase("E")) return new NamedTypedValue<>("string", "ExitSelection", null);

                if (Validator.isInt(userTaskSelection.getValue())) {
