...
public class InputHandler {
    ...
    public static <T> NamedTypedValue<T> getUserInput(String message, Class<T> targetClass) {
        while (true) {
            ...
            try {
                if ((targetClass == LocalDate.class && !Validator.isValidDatePattern(userInput))
                        || (targetClass == LocalTime.class && !Validator.isValidTimePattern(userInput))) {
                ...
                if (Validator.test(convertedValue, targetClass)) {
                ...
                System.out.println("Invalid input. Please try again with correct input and input type");
            }
        }
    }
}